...
public:
    ...
    bool Read_the_temperature(float &temperature);  //
    bool ReadHumidity(float &relative_humidity);    //Lecture de l'humidité relative
    float Temperature_Conversion(unsigned char msb, unsigned char lsb);
    float Humidity_Conversion(unsigned char msb, unsigned char lsb);
private:
